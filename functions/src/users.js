...
export async function signup(req, res) {
  ...
  if (!email || password.length < 6) {
    res.status(400).send({
      message:
        "Email and password are requires. Password must be 6 characters or long",
    });
    return;
  }
...
export async function login(req, res) {
  const { email, password } = req.body;
  if (!email || !password) {
    res.status(400).send({ message: "Email and password are requires." });
    return;
  }
